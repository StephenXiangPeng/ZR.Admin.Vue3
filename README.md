...
为了解决销售合同页面在填写表单数据时，切换到其他页面再返回导致数据丢失的问题，我们实施了表单数据持久化解决方案。

## 主要特性

1. **自动保存**: 表单数据会在用户输入时自动保存到 localStorage
2. **自动恢复**: 页面重新加载时会自动恢复之前保存的表单数据
3. **数据过期**: 保存的数据会在24小时后自动过期
4. **防抖保存**: 使用1秒防抖机制，避免频繁保存影响性能
5. **智能清理**: 成功提交或保存草稿后会自动清除保存的数据

## 使用方法

1. **正常使用**: 打开销售合同对话框，填写表单数据，系统会自动保存
2. **数据恢复**: 切换到其他页面再返回，系统会自动恢复之前的数据
