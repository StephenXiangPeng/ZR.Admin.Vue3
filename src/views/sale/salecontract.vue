	...
	<div>
		...
		<div style="border: 1px solid #e5e7eb; border-radius: 6px; overflow: hidden;">
			...
			<el-table :data="contractsTableData" style="width: 100%; table-layout: fixed;" stripe
				...
				<el-table-column prop="contractDate" label="合同日期" width="110"></el-table-column>
							...
							<el-col :span="6">
								<el-form-item label="目的地/港" prop="destinationPort">
									<el-input v-model="Newcontractform.destinationPort" style="width: 300px"
										:placeholder="'请输入目的地/港'" :disabled="isDisabled" size="default"></el-input>
								</el-form-item>
							</el-col>
							<el-col :span="6">
			...
			<el-tabs v-model="activeSearchProductTab">
				<el-tab-pane label="产品资料库" name="productInfoTab">
					<el-table :data="productDatatwo" :default-sort="{ prop: 'productCode', order: 'descending' }"
						style="width: 100%; table-layout: fixed;" @sort-change="handleSortChange"
						@row-dblclick="handleRowDblClick" stripe
						:header-cell-style="{ background: '#d1d5db', color: '#333', fontWeight: 'bold' }"
						:row-style="{ height: '20px' }" :cell-style="{ padding: '2px 0' }">
						<el-table-column prop="productCode" label="产品编号" sortable="custom" width="120" />
...
function handleSortChange({ prop, order }) {
	...
	GetProductInfoList(1, SearchProductpageSize.value); // 你现有的 pageSize 变量
}
...
const rules = reactive<FormRules<Newcontractform>>({
	...
	destinationPort: [{ required: true, message: '请输入目的地/港', trigger: 'change,blur' }],
...
const DeleteContract = (row) => {
	confirmDeleteWithCountdown('确定要删除该销售合同吗？此操作不可恢复。').then(() => {
		request.post('ApplyDeleteDocument/DelDocumentRequest/DelDocument', {
			DocumentID: row.id,
			DocumentType: DOCUMENT_TYPE_SALE_CONTRACT
		}).then(response => {
			if (response != null) {
				ElMessage({
...
const DOCUMENT_TYPE_SALE_CONTRACT = 3
